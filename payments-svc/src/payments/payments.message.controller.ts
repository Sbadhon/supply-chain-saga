...
export class PaymentsMessageController {
  ...
  async create(@Payload() data: { dto: CreatePaymentDto; traceId: string; idempotencyKey?: string }) {
  ...
  async retry(@Payload() data: { id: string; traceId: string; idempotencyKey?: string }) {
    const { id } = data;
    return this.paymentsService.retry(id);
  }

  @MessagePattern('payments.void')
  async voidPayment(@Payload() data: { id: string; traceId: string; idempotencyKey?: string }) {
    const { id } = data;
